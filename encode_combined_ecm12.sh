...
# Copy executable files into $home_folder/Preprocessing/App_{$BuildLabel}
...
do
  ...
  then
    ...
    then
      ...
      outputreconstructECM="$OutputFolder-ECM/rec-$name-$encfg-$QP.yuv"
    else
      outputreconstruct="\"\""
      outputreconstructECM="\"\""
    fi
    outputbitstream="$OutputFolder/str-$name-$encfg-$QP.bin"
    outputlogfile="$OutputFolder/log-$name-$encfg-$QP.txt"
    outputbitstreamECM="$OutputFolder-ECM/str-$name-$encfg-$QP.bin"
    outputlogfileECM="$OutputFolder-ECM/log-$name-$encfg-$QP.txt"
  else
    outputreconstruct="$OutputFolder/rec-$name-$encfg-$QP-$ReadStartFrame.yuv"
    outputreconstructECM="$OutputFolder-ECM/rec-$name-$encfg-$QP-$ReadStartFrame.yuv"
    ...
    outputbitstreamECM="$OutputFolder-ECM/str-$name-$encfg-$QP-$ReadStartFrame.bin"
    outputlogfileECM="$OutputFolder-ECM/log-$name-$encfg-$QP-$ReadStartFrame.txt"
  fi
  ...
  # execute commands
  fullcommandECM="$ReferenceAppFolder/EncoderApp $configsincommand -b $outputbitstreamECM -o $outputreconstructECM $FrameCountArgument $IntraPeriod -q $QP > $outputlogfileECM &"
